...
Amazon Web Services (`AWS`_) provides the preferred hosting for BE Platform.

**buildingenergy-platform** is a `Django project`_ and Django's documentation
...
The following AWS services are used for **buildingenergy-platform**:
...
.. code-block:: console
    ...
    $ git clone git@github.com:buildingenergy/buildingenergy-platform.git

enter the repo and install the python dependencies from `requirements.txt`_

.. _requirements.txt: https://github.com/buildingenergy/buildingenergy-platform/blob/master/requirements.txt

.. code-block:: console

    $ cd buildingenergy-platform
...
The BE Platform project relies on `redis`_ for both cache and message brokering, and
...
.. _script: https://github.com/buildingenergy/buildingenergy-platform/blob/master/bin/start_uwsgi.sh
