...
q1 = (
    ...
    'efficiency incentive program offered by your utility provider(s)?'
)
q1_new = (
    'Have you or the building owner participated in an energy '
    'efficiency incentive program offered by your utility provider(s) '
...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """fixes two typos, removes question 5 (radio) to make it the enum"""
        # fix typo
        orm.QuestionOption.objects.filter(
            option='substancial alterations'
        ).update(option='substantial alterations')
        # add more copy
        orm.SurveyQuestion.objects.filter(question=q1).update(question=q1_new)
        # move the question copy from 5 to 6
        sq = orm.SurveyQuestion.objects.get(pk=6)
        sq.question = (
            'Did the building undergo any major retrofit or physical changes '
            'during the last 2 years?'
        )
        sq.save()
        # move the question copy from 6 to 7
        sq = orm.SurveyQuestion.objects.get(pk=7)
        sq.question = 'If so, from what program or utility?'
        sq.save()
        # move question copy from 7 to (new obj) 8
        question = (
            'If there was funding or technology available, are there things '
            'that you feel your space needs to be improved or changed? (i.e. '
            'lighting, heating/cooling, general comfort, etc.)'
        )
        orm.SurveyQuestion.objects.create(
            survey=sq.survey,
            question=question,
            question_type=sq.question_type
        )
        # remove 5
        sq = orm.SurveyQuestion.objects.get(pk=5)
        sq.delete()

    def backwards(self, orm):
        orm.QuestionOption.objects.filter(
            option='substantial alterations'
        ).update(option='substancial alterations')
        orm.SurveyQuestion.objects.filter(question=q1_new).update(question=q1)
    ...
    models = {
        ...
        u'surveys.surveybuilding': {
            ...
            'date_collected': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime.now', 'null': 'True', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
