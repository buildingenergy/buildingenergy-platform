...
def _handle_ts_data(meter, timeseries_data):
    """Saves/updates timeseries data."""
    for ts in timeseries_data:
        t = None
        pk = ts.get('pk', None)
        if not pk:
            t = TimeSeries.objects.create(**ts)
        else:
            t = TimeSeries.objects.get(pk=pk)
            t.begin_time = ts.get('begin_time')
            t.end_time = ts.get('end_time')
            t.reading = ts.get('reading')
            t.cost = ts.get('cost')
        if t:
            t.meter = meter
...
def _handle_meter_data(bs, meters, business=None):
    """Saves/updates meter data.

    :param bs: BuildingSnapshot
    :param meters: list of dict
    :param business: Business
    :returns: BuildingSnapshot

    """
    for meter in meters:
        # Make mutations to our own copy of meter.
        meter = copy.deepcopy(meter)
        pk = meter.pop('id', None)
        m = None
        ts_data = meter.get('timeseries_data', [])
        energy_type = meter.get('energy_type', 'Electricity')
        meter['energy_type'] = convert_choice_data(
            energy_type, ENERGY_TYPES
        )

        energy_units = meter.pop('energy_units', 'kWh')
        meter['energy_units'] = convert_choice_data(
            energy_units, ENERGY_UNITS
        )
        if business:
            meter['business_id'] = business.pk
        if pk:
            m = Meter.objects.get(pk=pk)
            m.name = meter.get('name')
            m.energy_type = meter.get('energy_type')
            m.energy_units = meter.get('energy_units')
            m.building_snapshot.add(bs)
            if business:
                m.business = business
            m.save()

        elif meter:
            # Create the meter if one didn't already exist.
            # Remove this so we can safely create Meter model.
            if ts_data:
                del meter['timeseries_data']
            m = Meter.objects.create(**meter)

        if m:
            _handle_ts_data(m, ts_data)
        bs.meters.add(m)

    return bs


def _handle_business_contact(business, org, contact_data):
    """Update or create contact and update buisness."""
    from contacts.models import Contact, CONTACT_TYPES
    if not contact_data:
        return business

    contact_data = copy.deepcopy(contact_data)
    pk = contact_data.pop('id', None)
    contact_data = copy.deepcopy(contact_data)
    contact_data['contact_type'] = convert_choice_data(
        contact_data.get('contact_type'),
        CONTACT_TYPES
    )
    contact_data['super_organization_id'] = org.pk
    contact = None
    ...
    if contact:
        contact.super_organization = org
        contact.save()
...
def _handle_business_data(bs, businesses):
    """Saves/updates business data.
    :param bs: BuildingSnapshot inst.
    :param businesses: list of dict, business specific payload data.
    """
    for business in businesses:
        b = None
        pk = business.pop('id', None)
        contact_data = business.pop('contact', {})
        meter_data = business.pop('meters', [])
        if pk:
            b = Business.objects.get(pk=pk)
            b.name = business.get('name', None)
            b.address = business.get('address', None)
            b.occupancy = business.get('occupancy', None)
            b.gross_floor_area = business.get(
                'gross_floor_area', None
            )
            b.unit_number = business.get('unit_number', None)
            b.description = business.get('description', None)

            b.save()
        if (
            not business.get('name') or
            (not business and not contact_data and not meter_data)
        ):
            # We've removed our sentinal pk. If nothing else is specified,
            # don't create the business object.
            continue
        ...
        if not b:
            b = Business.objects.create(
                ...
                description=business.get('description', None),
                unit_number=business.get('unit_number', None),
                gross_floor_area=business.get(
                    'gross_floor_area', None
                ),
                occupancy=business.get('occupancy', None),
            )

        # TODO: this assumes there is only one contact per business.
        # I'd prefer to allow multiple contacts @alecklandgraf.
        b = _handle_business_contact(b, bs.super_organization, contact_data)
        _handle_meter_data(bs, meter_data, business=b)
        _handle_utility_data(
            bs,
            {k: v for k, v in business.items() if k in UTILITY_KEYS},
            business=b,
        )
...
def _handle_survey_data(bs, survey_data):
    """Save survey data.
    This functionality might as well live with the survey app, but the other
    building update functions are here, so it'll live with them for now.

    Expected payload:
        {
            'completion_time': 234,
            'comment': "Some general ideas, etc.",
            'date_collected': '2014-03-23',
            # Where the keys are the ids of the questions.
            'question_responses': {
                # text
                23: {'id': 234, 'answer': 'Coolio'},
                # radio
                234: {'id': None, 'answer': 'No'},
                # multi-select (QuestionOption.option as keys)
                33: {'HVAC': True, 'lighting retrofit': False}
            }
        }
    """
    pk = survey_data.pop('id', None)
    survey = None
    question_data = survey_data.pop('question_responses', {})
    if pk:
        survey = survey_models.Survey.objects.get(pk=pk)
    else:
        #  Something's wrong
        return bs

    if survey:
        sb, c = survey_models.SurveyBuilding.objects.get_or_create(
            survey=survey,
            canonical_building=bs.canonical_building
        )
        sb.date_collected = convert_datestr(
            survey_data.get('date_collected', None)
        )
        sb.comment = survey_data.get('comment', None)
        sb.completion_time = survey_data.get('completion_time', None)
        sb.save()
    for question_id, answer in question_data.items():
        answer_id = answer.pop('id', None)
        answer['question_id'] = question_id
        answer['canonical_building_id'] = bs.canonical_building.pk
        if answer_id:
            # Updating an existing answer.
            a = survey_models.SurveyAnswer.objects.get(pk=answer_id)
            a.answer = answer.get('answer', None)
            a.answer_bool = answer.get('answer_bool', None)
            a.canonical_building_id = bs.canonical_building.pk
        elif answer:
            # for multi-selects we should create a SurveyAnswers per checked
            if survey_models.SurveyQuestion.objects.get(
                pk=question_id
            ).question_type == survey_models.ENUM:
                del(answer['question_id'])
                del(answer['canonical_building_id'])
                for sa, checked in answer.items():
                    if checked:
                        survey_models.SurveyAnswer.objects.create(
                            question_id=question_id,
                            canonical_building_id=bs.canonical_building.pk,
                            answer=sa
                        )
                    else:
                        # remove any unchecked answers
                        survey_models.SurveyAnswer.objects.filter(
                            question_id=question_id,
                            canonical_building_id=bs.canonical_building.pk,
                            answer=sa
                        ).delete()
            else:
                # Create a new answer if there's an answer, but no previous id.
                a = survey_models.SurveyAnswer.objects.create(**answer)
        else:
            continue
...
UTILITY_KEYS = UTILITY_NAMES + UTILITY_ACCOUNTS


def _handle_utility_data(bs, utility_dict, business=None):
    """Save/Update Utility data."""
    for util_datum in utility_dict:
        if util_datum in UTILITY_NAMES:
            # Here we get or create a utility model.
            electric, gas = UTILITY_NAMES
            # Since America Saves only cares about these two types.
            if util_datum == electric:
                util_type = ELECTRICITY
            else:
                util_type = NATURAL_GAS
            util, _ = Utility.objects.get_or_create(
                name=utility_dict[util_datum],
                utility_type=util_type
            )
            if business:
                util.business.add(business)
            else:
                util.building_snapshot.add(bs)

    # Iterate through the list twice because we need to make sure the
    # Utilities are saved first before we can make the accounts
    for util_datum in utility_dict:
        if util_datum in UTILITY_ACCOUNTS:
            # get the utility based on type and building_snapshot
            util = None
            if util_datum == 'electric_provider_account_number':
                if business:
                    util = Utility.objects.get(
                        business=business, utility_type=ELECTRICITY
                    )
                else:
                    util = Utility.objects.get(
                        building_snapshot=bs, utility_type=ELECTRICITY
                    )
            elif util_datum == 'natural_gas_provider_account_number':
                if business:
                    util = Utility.objects.get(
                        business=business, utility_type=NATURAL_GAS
                    )
                else:
                    util = Utility.objects.get(
                        building_snapshot=bs, utility_type=NATURAL_GAS
                    )

            if not util:
                continue

            util_account, _ = UtilityAccount.objects.get_or_create(
                utility=util,
                account_number=utility_dict[util_datum],
                canonical_building=bs.canonical_building
            )
            # If there's no utility, there's no sense in saving the account.

    return bs


def _handle_audit_data(bs, building_dict):
    """Saves meter, business, utility data."""
    business_data = building_dict.get('businesses', [])
    meter_data = building_dict.get('meters', [])
    survey_data = building_dict.get('survey', {})
    bs = _handle_business_data(bs, business_data)
    bs = _handle_meter_data(bs, meter_data)
    bs = _handle_survey_data(bs, survey_data)
    bs = _handle_utility_data(
        bs, {k: v for k, v in building_dict.items() if k in UTILITY_KEYS}
    )
...
def update_building(old_snapshot, updated_values, user, *args, **kwargs):
    ...
    # Add the org the old snapshot had.
    new_snapshot.super_organization = old_snapshot.super_organization
    new_snapshot.save()

    new_snapshot.businesses.add(*old_snapshot.businesses.all())
    new_snapshot.meters.add(*old_snapshot.meters.all())

    # Save new audit data values and attach linkages to this new snapshot.
    # Here we add the business, meter, timeseries data related to this building
    _handle_audit_data(new_snapshot, updated_values)
...
class Meter(models.Model):
    ...
    business = models.ForeignKey(
        'Business', related_name='meters', null=True, blank=True
    )
    ...
    def to_dict(self):
        """serializes Meter to python dict, JSON safe"""
        m = obj_to_dict(self)
        m['energy_type'] = self.get_energy_type_display()
        m['energy_units'] = self.get_energy_units_display()
        return m
...
class TimeSeries(models.Model):
    ...
    occupancy = models.DecimalField(max_digits=5, decimal_places=2, null=True)
    meter = models.ForeignKey(
        Meter, related_name='timeseries_data', null=True, blank=True
    )

    def to_dict(self):
        """serializes TimeSeries to python dict, JSON safe"""
        m = obj_to_dict(self)
        m['cost'] = float(self.cost) if self.cost is not None else None
        if self.occupancy is not None:
            m['occupancy'] = float(self.occupancy)
        return m
...
class Business(models.Model):
    ...
    owner = models.ForeignKey('contacts.Contact', null=True, blank=True)
    gross_floor_area = models.FloatField(null=True, blank=True)
    building_snapshot = models.ManyToManyField(
        BuildingSnapshot, related_name='businesses', null=True, blank=True
    )
    occupancy = models.FloatField(
        null=True,
        help_text='the occupancy percentage * 100, range 0.0 to 100.0'
    )
...
class UtilityAccount(models.Model):
    ...
    canonical_building = models.ForeignKey(CanonicalBuilding)
...
class Utility(models.Model):
    name = models.CharField(max_length=100)
    address = models.CharField(max_length=255, null=True, blank=True)
    utility_type = models.IntegerField(max_length=3, choices=ENERGY_TYPES)
    business = models.ManyToManyField(
        Business, related_name='utilities', null=True, blank=True
    )
    building_snapshot = models.ManyToManyField(
        BuildingSnapshot, related_name='utilities', null=True, blank=True
    )
    accounts = models.ManyToManyField(
        CanonicalBuilding, through='UtilityAccount'
    )
