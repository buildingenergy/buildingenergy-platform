...
def home(request):
    ...
    # hack to get america saves users a different logo (sorry, Aleck)
    # see seed templates _header
    is_america_saves_user = False
...
def get_building(request):
    ...
    # Add in the survey/meter specicific data
    building_dict['survey'] = build_survey_payload(building)
    building_dict['meters'] = build_meter_payload(building)
    building_dict['businesses'] = build_business_payload(building)
    building_dict.update(build_utility_payload(building))
...
def create_building(request):
    ...
    :POST: Expects organization_id and a building object in the body.
    ...
    if request.method != 'POST':
        return {'status': 'error', 'message': 'only http POST allowed'}
    body = json.loads(request.body)
    org = Organization.objects.get(pk=body.get('organization_id'))
    bs = BuildingSnapshot.objects.create(super_organization=org)
    building = body.get('building', {})
    for k, v in building.items():
        setattr(bs, k, v)
        setattr(bs, '%s_source' % k, bs)
    canon = CanonicalBuilding.objects.create(
        canonical_snapshot=bs
    )
    bs.canonical_building = canon
    bs.save()
    AuditLog.objects.log_action(
        request, canon, org.pk, action_note='Created building.'
    )
